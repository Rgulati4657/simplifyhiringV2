...
export function SelectedCandidatesManager() {
  ...
  const fetchSelectedCandidates = async () => {
    setLoading(true);
    try {
      console.log('Calling database function: get_company_selected_candidates_details');

      // We call our new function. No .select() is needed because the
      // function already prepared the complete JSON data for us.
      const { data, error } = await supabase
        .rpc('get_company_selected_candidates_details');
      ...
      // We need to extract the 'candidate_data' property from each item.
      const formattedData = data ? data.map(item => item.candidate_data) : [];

      console.log('Fetched and formatted data via RPC:', formattedData);
      console.log('Number of selected candidates:', formattedData.length);
      
      // This formatted data now perfectly matches the structure your component expects.
      setSelectedCandidates(formattedData as any);
