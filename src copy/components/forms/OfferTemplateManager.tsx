...
const OfferTemplateManager = ({ trigger, onTemplateUploaded }: OfferTemplateManagerProps) => {
  ...
  // };
    ...
    const fetchTemplates = async () => {
    // We no longer need the 'profile' check, as the database function handles security.
    try {
      setLoading(true);
      console.log('Calling database function: get_company_offer_templates');

      // Call the new database function using .rpc()
      // We can still chain methods like .order() to the result.
      const { data, error } = await supabase
