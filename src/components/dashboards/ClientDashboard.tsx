...
const [applications, setApplications] = useState([]);
 ...
 const [editJobModal, setEditJobModal] = useState<{ open: boolean; id: string | null }>({
    open: false,
    id: null,
  });
 ...
 console.log(`[ClientDashboard] Current viewingJobId is:`, viewingJobId);
  ...
  const JOBS_PER_PAGE = 6; // You can change this number

   const { paginatedJobs, totalPages } = useMemo(() => {
    let processedData = [...recentJobs];
    ...
    const calculatedTotalPages = Math.ceil(processedData.length / JOBS_PER_PAGE);

    // Slice the filtered data to get only the items for the current page
    const startIndex = (mainListCurrentPage - 1) * JOBS_PER_PAGE;
    const paginatedItems = processedData.slice(startIndex, startIndex + JOBS_PER_PAGE);

    return { paginatedJobs: paginatedItems, totalPages: calculatedTotalPages };
  }, [recentJobs, mainListSearchTerm, mainListCurrentPage]);
...
const fetchDashboardData = async () => {
  ...
  setLoading(true);
  try {
    console.log('Calling secure database function: get_company_jobs_with_details');

    // This is the single, powerful RPC call that replaces the old .from('jobs')
    // It securely gets ONLY the jobs for the logged-in admin's company.
    const { data: jobsData, error: jobsError } = await supabase
      .rpc('get_company_jobs_with_details') // <-- THE KEY CHANGE
      .select(`
        *,
        companies (name),
        job_applications (
    ...
    // The rest of your logic does not need to change at all.
    // It will now work correctly because jobsData is correctly filtered.
    if (jobsData) {
      console.log("SUCCESS: Fetched company-specific job data:", jobsData);
  ...
  return (
    <DashboardLayout title="Client Dashboard" actions={dashboardActions}>
      <div className="space-y-8 animate-fade-in">
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            setMainListCurrentPage(1); // Reset to page 1 on new search
          }}
          className="pl-10"
        />
      </div>

      <div className="space-y-4 min-h-[500px]"> {/* Added a min-height */}
        {/* --- 2. MAP OVER THE NEW PAGINATED LIST --- */}
        {paginatedJobs.length > 0 ? (
          paginatedJobs.map((job: any) => (
            // Your existing job card is used here, no changes to its internal design
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-4">
                              ...
                              <div className="flex items-center space-x-1">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => setEditJobModal({ open: true, id: job.id })}>
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground mb-4"> {mainListSearchTerm ? "No jobs match your search." : "No jobs posted yet"}</p>
      ...
      <EditJobModal
          ...
          onOpenChange={(open) => setEditJobModal({ ...editJobModal, open })}
          onJobUpdated={fetchDashboardData} // Reuse your existing data refresh function!
        />
