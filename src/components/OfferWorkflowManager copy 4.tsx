...
// PASTE THIS FINAL AND DEFINITIVE VERSION. IT HANDLES OVERWRITING.
// voilates row sequrity 
// const generateOfferWithFields = async () => {
//     if (!templateFile || !selectedWorkflowForOffer) {
//       toast({
//         title: "Error",
...
//       // Download the generated PDF content as a blob
...
//       // Step 2: Create a PERMANENT, PREDICTABLE file path.
//       // NO MORE TIMESTAMPS! This is the key to overwriting.
...
//       // `upsert: true` will automatically OVERWRITE the old file if it exists.
//       const { error: uploadError } = await supabase.storage
//         .from('offer-letters')
//         .upload(filePath, pdfBlob, {
//           contentType: 'application/pdf',
//           upsert: true // This is the magic for overwriting!
//         });
...
//       // Step 4: Get the public URL for the file we just uploaded/updated.
...
//       // Step 5: Save this permanent URL and other details to the 'offers' table.
//       // (Includes all previous fixes for date and data types)
...
//       // Step 6: Update the 'offer_workflow' table as well.
...
//       // Step 7: Show the preview using the permanent Supabase URL.
...
// PASTE THIS VERSION TO DEBUG THE AUTHENTICATION STATE
...
// PASTE THIS FINAL, REVISED FUNCTION. THIS IMPLEMENTS YOUR FRIEND'S SUGGESTION.
// nice work 
// const generateOfferWithFields = async () => {
//     if (!templateFile || !selectedWorkflowForOffer) {
//       toast({
...
//     setGeneratingOffer(true);
//     try {
//       // Step 1: Generate the PDF file content using the service, but DON'T let the service save it.
//       // We expect the service to return the raw PDF file (blob).
//       const pdfBlob = await offerApiService.downloadFile(
//         (await offerApiService.generateOfferLetter({
//           template_file: templateFile,
//           data: fieldValues,
//           output_format: 'pdf' // We only need the PDF
//         })).files.pdf
//       );

//       if (!pdfBlob) {
//         throw new Error("API service did not return a valid PDF file.");
//       }

//       // Step 2: Upload THIS PDF to OUR Supabase bucket ('offer-letters').
//       const filePath = `public/offer-${selectedWorkflowForOffer.job_application_id}-${Date.now()}.pdf`;
//       const { error: uploadError } = await supabase.storage
//         .from('offer-letters') // Your bucket name
//         .upload(filePath, pdfBlob, {
//           contentType: 'application/pdf',
//           upsert: true
//         });
...
//       // Step 3: Get the public URL for the file we just uploaded to OUR bucket.
//       const { data: urlData } = supabase.storage
//         .from('offer-letters')
//         .getPublicUrl(filePath);

//       const supabaseFileUrl = urlData.publicUrl; // This is the URL we will save everywhere.

//       // Step 4: Save the new Supabase URL and other data into the `offers` table.
//       // (Includes previous fixes for date formatting)
//       let formattedStartDate = null;
//       if (fieldValues.start_date && typeof fieldValues.start_date === 'string') {
//         const parts = fieldValues.start_date.split(/[/.-]/);
//         if (parts.length === 3) {
...
//         start_date: formattedStartDate,
//         benefits: fieldValues.benefits || [],
//         offer_letter_url: supabaseFileUrl, // Save the new Supabase URL
//         status: 'draft' as const,
//       };
...
//       // Step 5: Also update the `offer_workflow` table with the new Supabase URL.
//       const { error: workflowError } = await supabase
//         .from('offer_workflow')
//         .update({
//           offer_letter_url: supabaseFileUrl, // Save the new Supabase URL
...
//       // Step 6: Show the preview using the new Supabase URL.
//       setPreviewPdfUrl(supabaseFileUrl);
//       setGeneratedOfferData(fieldValues);
...
// PASTE THIS FINAL, CORRECTED FUNCTION INTO YOUR OfferWorkflowManager.tsx FILE
...
//     setGeneratingOffer(true);
//     try {
//       // Date formatting logic from before
//       let formattedStartDate = null;
//       if (fieldValues.start_date && typeof fieldValues.start_date === 'string') {
//         const parts = fieldValues.start_date.split(/[/.-]/);
//         if (parts.length === 3) {
//             if (parts[0].length === 4) {
//                 formattedStartDate = fieldValues.start_date; // Already YYYY-MM-DD
//             } else {
//                 const day = parts[0].padStart(2, '0');
//                 const month = parts[1].padStart(2, '0');
//                 const year = parts[2];
//                 formattedStartDate = `${year}-${month}-${day}`; // Convert to YYYY-MM-DD
//             }
//         }
//       }

//       // Step 1: Generate the letter via the API Service
//       const response = await offerApiService.generateOfferLetter({
//         template_file: templateFile,
//         data: fieldValues,
//         output_format: 'both'
...
//       if (!response.success || !response.files?.pdf) {
//         throw new Error(response.message || 'API failed to return a PDF file ID');
//       }

//       const permanentFileId = response.files.pdf; // This is the ID from the ApiService

//       // Step 2: Save the final data to the `offers` table
//       const salaryString = String(fieldValues.salary || '0').replace(/[^0-9.-]+/g, '');
//       const salaryAmount = parseFloat(salaryString) || 0;
...
//         start_date: formattedStartDate,
//         benefits: fieldValues.benefits || [],
//         signing_bonus: parseFloat(String(fieldValues.signing_bonus || '0')),
//         probation_period_months: parseInt(String(fieldValues.probation_period_months || '0')),
//         notice_period_days: parseInt(String(fieldValues.notice_period_days || '0')),
//         offer_letter_url: permanentFileId, // URL saved in 'offers' table
//         status: 'draft' as const,
//       };
...
//       // Step 3: Update the `offer_workflow` table (THIS IS THE FIX)
//       const { error: workflowError } = await supabase
//         .from('offer_workflow')
//         .update({
//           offer_letter_url: permanentFileId, // <-- THE FIX: Save the URL here too
//           offer_details: {
//             ...fieldValues,
//             pdf_file_id: permanentFileId,
//             docx_file_id: response.files.docx,
//           },
//           offer_generated_at: new Date().toISOString(),
//           updated_at: new Date().toISOString()
...
//       // Step 4: Prepare the preview for the UI
//       const pdfBlob = await offerApiService.downloadFile(permanentFileId);
...
// };
  ...
  // FIND THE OLD `regenerateOfferWithChanges` AND REPLACE IT WITH THIS NEW VERSION
